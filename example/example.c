...
tx_msg_func(void *arg)
{
	 ...
	 */
	fail_unless(msg->self == module.tx_thread && pthread_self() == module.thread);
...
module_msg_func(void *arg)
{
	 ...
	 * Also here you can print msg and make sure of this
	 */
	fail_unless(msg->self == module.thread && pthread_self() == module.tx_thread);
	msg->self = pthread_self();
	if (msg->counter == MAGIC_MSG_COUNTER)
		fprintf(stderr, "module_msg_func called\n");
	if (!msg->stop && xtm_msg_probe(module.in) == 0)
	{
		fail_unless(xtm_fun_dispatch(module.in, tx_msg_func, msg, 0) == 0);
	}
	else
	{
		free(msg);
...
enqueue_message(bool stop)
{
	...
	{
		fail_unless(xtm_fun_dispatch(module.out, module_msg_func, msg, 0) == 0);
	}
	else
	{
		free(msg);
...
tx_fiber_func(va_list arg)
{
	...
	{
		fail_unless(coio_wait(pipe_fd, COIO_READ, DBL_MAX) & COIO_READ);
...
main_module_func(void *arg)
{
	...
	while (1)
	{
		...
		int max = (pipe_fd > module.stop_fds[0] ? pipe_fd : module.stop_fds[0]);
		...
		{
			uint64_t tmp;
			ssize_t read_bytes = read(module.stop_fds[0], &tmp, sizeof(tmp));
			fail_unless(read_bytes == sizeof(tmp) && tmp == STOP_MAGIC);
			/*
...
cfg(lua_State *L)
{
	 ...
	 */
	fail_unless(pthread_create(&module.thread, NULL, main_module_func, NULL) == 0);
	/*
	 ...
	 */
	fail_unless((module.tx_fiber = fiber_new("tx_fiber", tx_fiber_func)) != NULL);
